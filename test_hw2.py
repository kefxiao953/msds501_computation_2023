...
def test_return_title_id_original_title_dict():
    """
    Test case for return_title_id_original_title_dict()
    """
    input_file = './imdb_titles_medium.tsv'
    delimiter = '\t'
    keys = ['title', 'region', 'language', 'types']
    ...
    movie_list = create_movie_list(input_file, delimiter, keys,
                                   null_value, title_id, information)
    title_id_original_title_dict = return_title_id_original_title_dict(
                                   movie_list, title_id)

    assert len(title_id_original_title_dict) == 3350\
           and title_id_original_title_dict['tt0000003'] == 'Pauvre Pierrot'\
           and title_id_original_title_dict['tt0000447'] == 'Life of an American Fireman'


def test_return_information_element_set():
    """
    Test case for return_information_element_set()
    """
    input_file = './imdb_titles_medium.tsv'
    delimiter = '\t'
    keys = ['title', 'region', 'language', 'types']
    ...
    movie_list = create_movie_list(input_file, delimiter, keys, null_value,
                                   title_id, information)
    assert return_information_element_set(movie_list=movie_list,
                                          key='types') == {'alternative',
                                                           'imdbDisplay',
                                                           'original',
                                                           'festival',
                                                           'dvd',
                                                           ...
                                                           'video',
                                                           'working'}\
           and return_information_element_set(movie_list=movie_list,
                                              key='language',
                                              alt='types') == {'fa', 'qbn',
                                                               'ja', 'fr',
                                                               'hi', 'bg',
